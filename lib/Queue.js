...
export class Queue {
    ...
    handleEvent = (event) => {
      ...
      if ((event.consumer_tag != this.consumer_tag) || (this.consumer_tag == '')){ return; }
	...
	declare(getsuccessCallback, geterrorCallback){
		
        this.rabbitmqconnection.declareQueue(this.name, 		
		getsuccessCallback, geterrorCallback);		
    }
	
    bind(exchange, routing_key = ''){
        this.rabbitmqconnection.bindQueue(exchange.name, this.name, routing_key, null);
    }
	
	consume(parameters, args, successCallback, errorCallback){

        if(this.consumer_tag == ''){

            this.rabbitmqconnection.basicConsume(this.name, 
                                                parameters,
                                                args,      
                                                successCallback,
                                                errorCallback);
    ...
    cancel_consume(successCallback, errorCallback){

        if(this.consumer_tag == ''){

            this.rabbitmqconnection.basicCancel(this.consummer_tag,      
                                                successCallback,
                                                errorCallback);
        }
    }	

    set_consumer_tag(consumer_tag){
    ...
    unbind(exchange, routing_key, args){
        this.rabbitmqconnection.unbindQueue(exchange.name, this.name, routing_key, args);
    }
    ...
    basicAck(delivery_tag, multiple) {
        ...
        this.rabbitmqconnection.basicAck(delivery_tag, multiple);
