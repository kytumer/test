...
public class RabbitMqExchange {
	...
	public void exchangeDeclarePassive(){
			
		try {
             this.channel.exchangeDeclarePassive(this.name); 		

        } catch (IOException e){
            Log.e("Exchange not found.Declaring exchange.", "Exchange error " + e);
			e.printStackTrace();
        }
	}	
	
	
    public void publish(String message, String routing_key, boolean mandatory, ReadableMap message_properties){
        try {
            ...
            if (message_properties != null){
                 try {
                    ...
                    if (message_properties.hasKey("timestamp")&& message_properties.getType("timestamp") == ReadableType.Boolean){
						if (message_properties.getBoolean("timestamp")){
							Date date = new Date();
                            date.getTime();
            ...
            this.channel.basicPublish(this.name, routing_key, mandatory, properties.build(), message_body_bytes);
        } catch (Exception e){
